...
public:
	...
	template <class T1,class T2>
	void warpImageBicubicMask(Image<T>& mask,const Image<T>& ref,Image<T>& output,const Image<T1>& imdx,const Image<T1>& imdy, const Image<T1>& imdxdy,const Image<T2>& vx,const Image<T2>& vy) const;

	template <class T1>
	void warpImageBicubicMask(Image<T>& mask,const Image<T>& ref,Image<T>& output,const Image<T1>& vx,const Image<T1>& vy) const;
...
template <class T>
template <class T1>
void Image<T>::warpImageBicubicMask(Image<T>& mask,const Image<T>& ref,Image<T>& output,const Image<T1>& vx,const Image<T1>& vy) const
{
	double dfilter[3] = {-0.5,0,0.5};
	DImage imdx,imdy,imdxdy;
	imfilter_h(imdx,dfilter,1);
	imfilter_v(imdy,dfilter,1);
	imdx.imfilter_v(imdxdy,dfilter,1);
	warpImageBicubicMask(mask,ref,output,imdx,imdy,imdxdy,vx,vy);
}
...
template <class T>
template <class T1,class T2>
void Image<T>::warpImageBicubicMask(Image<T>& mask,const Image<T>& ref,Image<T>& output,const Image<T1>& imdx,const Image<T1>& imdy,const Image<T1>& imdxdy,
																		const Image<T2>& vx,const Image<T2>& vy) const
{
	T* pIm = pData;
	const T1* pImDx = imdx.data();
	...
	{
		output.allocate(width,height,nChannels);
		mask.allocate(width,height,nChannels);
	}
	double a[4][4];
	int offsets[2][2];
		...
		for(int j = 0;j<width;j++)
		{
			...
			if(x<0 || x>imWidth-1 || y<0 || y>imHeight-1)
			{
				for(int k = 0; k<nChannels;k++)
				{
					...
					mask.pData[offset*nChannels+k] = 0;
				}
			...
			for(int k = 0;k<nChannels;k++)
			{
				...
				mask.pData[offset*nChannels+k] = 1;
			}
		}
}
																		...
																		const Image<T2>& vx,const Image<T2>& vy) const
{
	...
	/*	PoissonBlender poissonBlende(src, dst, Mask);
		cv::Mat dst1(width, height, CV_8UC(1));
		poissonBlende.seamlessClone(dst1, 0, 0); 
		
		for(int i  = 0; i<height; i++)
			for(int j = 0;j<width;j++)
			{
				int offset = i*width+j;
				output.pData[offset] =  dst1.at<uchar>(i, j);
			}     */
}
